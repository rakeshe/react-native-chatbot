...
class ChatBot extends Component {
  ...
  constructor(props) {
    ...
    this.addNewStep = this.addNewSteps.bind(this);
  ...
  /**
   * Parses incoming newStepData, updates state and returns parsed steps
   */
  addNewSteps(newStepData) {
    const newSteps = {};
    const { steps } = this.state;

    /** @refactor: use common method to get defaults  */
    const {
      botDelay,
      botAvatar,
      botBubbleColor,
    ...
    _.each(newStepData, step => {
      // Skip if this step already exists
      if (steps[step.id]) {
        return;
      }
